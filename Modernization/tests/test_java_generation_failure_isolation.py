...
class JavaGenerationFailureIsolationTests(unittest.TestCase):
    ...
    def test_java_reconciliation_is_not_called_for_dotnet_generation(self):
        output = {"Demo/backend/Demo.cs": "public sealed class Demo {}\n"}
        with patch(
            "services.build_runner.run_build",
            return_value=BuildResult(True, "dotnet", {}),
        ), patch(
            "services.modernizer.build_artifacts._reconcile_java_generation_output",
            side_effect=AssertionError("Java reconciliation reached .NET generation"),
        ) as java_reconcile:
            result = _pf_run_build_and_repair(
                output, "Demo", "csharp", False, "project", "", "", "model", "",
